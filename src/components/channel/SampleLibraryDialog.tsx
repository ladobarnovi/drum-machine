...
export default function SampleLibraryDialog({
  ...
  return (
    <Modal
      ...
      subtitle={`Loading into ${channelLabel}`}
      closeLabel="Close sample library"
      onClose={onClose}
      footer={
        <>
    ...
    >
      {/* Sticky rather than a band of its own above the scroller, now that the
          shell owns the one scrolling box: the field has to stay reachable
          with the list run down past it, which is what it was doing before by
          sitting outside it. */}
      <div className="bg-surface sticky -top-4 z-20 -mx-4 -mt-4 mb-3 px-4 pt-4 pb-3">
        <input
          type="search"
          value={query}
        ...
        <div className="flex flex-col gap-4">
          {results.map((category) => (
            <section key={category.id}>
              {/*
                Sticky, so the machine a row belongs to stays on screen once
                the list is longer than the panel and scrolling has carried its
                heading off the top. Offset below the search field above, which
                claims the top of the same scroller.
              */}
              <div className="bg-surface sticky top-9 z-10 pb-1.5">
                <h3 className="text-xs font-semibold">{category.name}</h3>
                <p className="text-muted text-[10px]">{category.description}</p>
              </div>
                  ...
                  return (
                    <li key={sample.id}>
                      <button
                        ...
                        // Named for what pressing it does rather than left to
                        // read out as its contents, which under a heading
                        // nobody has to be told twice would be "Kick" and
                        // then, on the one already in the slot, "Kick Loaded".
                      ...
                      >
                        ...
                        {/* Only on the one already in the slot: the fill says
                            which row that is, and this says what the fill
                            means. */}
                        {current && (
