/**
 * The Web Audio graph: the master chain, the per-hit voice, and everything that
 * shapes either.
 *
 * Split out of `hooks/useSampleBank`, which is where all of this used to live —
 * a thousand lines of audio code above the hundred-line hook that closes over
 * it. Nothing here touches React: it takes an `AudioContext` and returns or
 * mutates nodes, so it can be read, and reasoned about, as audio code alone.
 */
 ...
 */
export const REVERB_IMPULSE_SETTLE_MS = 120;
 ...
 */
export function createRandomLfoTable(context: AudioContext): AudioBuffer {
 ...
 */
export function createLfoSource(
 ...
 */
export function startLfoSource(source: LfoSource, time: number) {
  if ("buffer" in source) {
 ...
 * is a step away.
 */
export function ensureChannelLfo(
  context: AudioContext,
 ...
 */
export function releaseChannelLfo(
  lfos: Map<string, ChannelLfoNodes>,
  channelId: string,
) {
 ...
 */
export function meterForChannel(
  context: AudioContext,
 ...
 * slope one.
 */
export function appendCutFilter(
  context: AudioContext,
  input: AudioNode,
  type: "highpass" | "lowpass",
  frequency: number,
...
export function createMasterChain(context: AudioContext): MasterChain {
 ...
 */
export function applyReverbImpulse(
  context: AudioContext,
  chain: MasterChain,
  reverb: MasterReverb,
) {
